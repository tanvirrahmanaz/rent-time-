    ...
    <nav className="bg-white/98 backdrop-blur-lg shadow-xl border-b border-slate-200/50 sticky top-0 z-50">
            ...
            <Link to="/" className="flex items-center space-x-3 group">
              <div className="relative">
                ...
                <div className="relative bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 p-3 rounded-xl shadow-lg">
                  <svg className="h-7 w-7 text-white" xmlns="http://www.w3.org/2000/svg" fill="none"
          ...
          <div className="hidden md:flex items-center space-x-4">
            {loading ? (
                    ...
                    <div className="py-2">
                      ...
                      {isAdmin && (
                        ...
                        >
                          <svg className="w-4 h-4 mr-3 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                                  d="M12 8c-1.657 0-3 1.567-3 3.5S10.343 15 12 15s3-1.567 3-3.5S13.657 8 12 8zm0-5C6.477 3 2 7.477 2 13s4.477 10 10 10 10-4.477 10-10S17.523 3 12 3z" />
                          </svg>
                          Admin Dashboard
                        </Link>
                      )}
              ...
              <div className="flex items-center space-x-3">
                <Link to="/login" className="text-slate-700 hover:text-slate-900 font-semibold text-sm px-6 py-3 rounded-xl hover:bg-slate-100 transition-all duration-300">
                  Login
                </Link>
                <Link to="/signup" className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-6 py-3 rounded-xl text-sm font-semibold shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5">
                  Sign Up
                </Link>
              </div>
            )}
      ...
      <div className={`md:hidden transition-all duration-500 ease-in-out border-t border-slate-200/50 backdrop-blur-lg ${isMobileMenuOpen ? 'max-h-screen py-6 opacity-100' : 'max-h-0 py-0 opacity-0 overflow-hidden'}`}>
        <div className="px-4 space-y-2">
          {publicNavLinks.map((link) => (
            <NavLink key={link.text} to={link.to} onClick={() => setIsMobileMenuOpen(false)}
              className={({ isActive }) =>
          ...
          {currentUser && [...privateNavLinks, ...(isAdmin ? [{ to: '/admin', text: 'Admin Dashboard' }] : [])]
            .map((link) => (
              <NavLink key={link.text} to={link.to} onClick={() => setIsMobileMenuOpen(false)}
                className={({ isActive }) =>
                  `block px-4 py-3 rounded-xl text-base font-semibold transition-all duration-300 ${
            ...
            <div className="space-y-4">
              ...
              {isAdmin && (
                <Link to="/admin" onClick={() => setIsMobileMenuOpen(false)}
                      className="block text-center bg-gradient-to-r from-fuchsia-600 to-indigo-600 text-white px-6 py-4 rounded-xl text-base font-semibold shadow-lg transition-all">
                  Admin Dashboard
                </Link>
              )}
